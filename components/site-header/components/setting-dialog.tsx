  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="flex h-[80vh] max-h-[80vh] flex-col rounded-lg border-border bg-card text-card-foreground mx-auto px-4 sm:max-w-[640px]">
        <DialogHeader className="shrink-0 space-y-1.5 border-b border-border pb-4">
        ...
        <ScrollArea className="grow py-6 pr-6">
          <Accordion type="multiple" className="w-full space-y-4">
            {localCategories.map((category, catIndex) => (
              <AccordionItem
                value={`item-${catIndex}`}
                key={catIndex}
                className="rounded-lg border border-border shadow-xs transition-colors hover:bg-accent/5"
              >
                ...
                <AccordionContent className="pb-4 pl-6 pr-2 pt-2">
                  <div className="space-y-4">
                    {category.items.map((item, linkIndex) => (
                      <div
                        key={linkIndex}
                        className="relative grid grid-cols-1 gap-3 rounded-md border border-border bg-card/50 p-4 shadow-xs transition-colors hover:bg-accent/5"
                      >
                        <Button
                          variant="ghost"
                          size="icon"
        ...
        <DialogFooter className="shrink-0 flex-col-reverse gap-2 border-t border-border pt-4 sm:flex-row sm:justify-end">
          <Button
            variant="destructive"
